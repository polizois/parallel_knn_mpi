...
*/
/*
    This iteration of knn uses the BLOCKING communication commands of MPI :
		- MPI_Send
		- MPI_Recv
		So before before a process can start with the knn search, it has to wait
		every time for the comunications, between it and its neighbours, to finish.
*/
...
int main(int argc, char *argv[])
{
    ...
    if (rank == (numtasks - 1))  next = 0;
		...
		//Loading bin files to the apropriate arrays we allocated memory for
		...
		//Preparing the points, labels and rank to be sent to the next task
		...
		{
			// Concecutive tasks need to have different order of send and receive
			// so that they dont block eachother
			if(rank%2)
			{
				MPI_Recv(&(pointsRecv[0][0]), coordNum*taskPointNum, MPI_DOUBLE, prev, tag1, MPI_COMM_WORLD, &stats[0]);
				MPI_Recv(&(labelsRecv[0]), taskPointNum, MPI_DOUBLE, prev, tag2, MPI_COMM_WORLD, &stats[1]);
				MPI_Recv(&rankRecv, 1, MPI_INT, prev, tag3, MPI_COMM_WORLD, &stats[2]);

				MPI_Send(&(pointsSend[0][0]), coordNum*taskPointNum, MPI_DOUBLE, next, tag1, MPI_COMM_WORLD);
				MPI_Send(&(labelsSend[0]), taskPointNum, MPI_DOUBLE, next, tag2, MPI_COMM_WORLD);
				MPI_Send(&rankSend, 1, MPI_INT, next, tag3, MPI_COMM_WORLD);
			}
			else
			{
				MPI_Send(&(pointsSend[0][0]), coordNum*taskPointNum, MPI_DOUBLE, next, tag1, MPI_COMM_WORLD);
			...
			knnSearch(points, myPointNum, pointsRecv, labelsRecv, otherStart, otherPointNum, myData, nbrNum, coordNum);
